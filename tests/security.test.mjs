...
test("security controls reject cross-site writes, spoofed files and IP-header bypasses", async () => {
  ...
  assert.equal((caddy.match(/header_down -WWW-Authenticate/g) ?? []).length, 2);
  ...
  assert.match(compose, /MTX_AUTHINTERNALUSERS_0_USER/);
  assert.doesNotMatch(compose, /MTX_AUTHINTERNALUSERS_1_/);
  ...
  assert.match(mediaMtx, /authInternalUsers:[\s\S]*?- user: disabled[\s\S]*?action: publish[\s\S]*?path: "~\^\$"/);
  assert.match(mediaMtx, /- user: any\s+pass:\s+ips: \[\][\s\S]*?action: read\s+path: "~\^radio\/\[a-z0-9_-\]\{3,40\}\$"/);
